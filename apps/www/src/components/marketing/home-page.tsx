	...
	return (
		<MarketingShell locale={locale} page="home">
			...
			<MarketingSection
				...
				innerClassName="grid gap-7 py-20 sm:py-24 lg:grid-cols-12 lg:gap-10 lg:py-28"
			>
				<p className="marketing-kicker text-[var(--yonaris-slate)]/70 lg:col-span-3">{content.navigation[3].label}</p>
				<div className="lg:col-span-8 lg:col-start-5">
					<h2 className="marketing-display max-w-[17ch] text-[clamp(2.35rem,5vw,5.25rem)] leading-[0.98] font-medium tracking-[-0.045em] text-balance">
						{content.brandThesis}
					</h2>
					<p className="mt-7 max-w-[46rem] text-base leading-7 text-[var(--yonaris-slate)]/78 sm:text-lg sm:leading-8">
						{content.companyDefinition}
					</p>
				</div>
			</MarketingSection>
			...
			>
				...
				<div className="mt-20 grid border-t border-[var(--yonaris-ink)]/18 sm:grid-cols-2 lg:mt-28 lg:grid-cols-4">
					{content.capabilities.map((capability, index) => (
						<article
							key={capability.name}
							className="min-h-72 border-b border-[var(--yonaris-ink)]/14 py-7 sm:border-r sm:px-6 lg:min-h-80 lg:border-b-0 first:sm:pl-0 last:sm:border-r-0"
						>
							...
							<p className="mt-4 max-w-[17rem] text-sm leading-6 text-[var(--yonaris-slate)]/70">
			...
			<MarketingSection
				...
				innerClassName="relative py-24 sm:py-32 lg:py-40"
			>
				<div className="pointer-events-none absolute -right-24 bottom-0 h-[70%] w-[58%] opacity-45 max-lg:hidden">
			...
			>
				...
				<ol className="mt-20 border-t border-[var(--yonaris-ink)]/18 lg:ml-[25%] lg:mt-28">
					{content.method.steps.map((step, index) => (
						<li
							key={step.name}
							className="grid gap-5 border-b border-[var(--yonaris-ink)]/14 py-7 sm:grid-cols-12 sm:items-start sm:gap-8"
						>
							...
							<p className="max-w-[34rem] text-sm leading-6 text-[var(--yonaris-slate)]/68 sm:col-span-6 sm:col-start-7">
								{step.description}
							</p>
						</li>
					))}
				...
				<div className="mt-10 lg:ml-[25%]">
					<MarketingLink href={isZh ? "/zh/approach" : "/approach"} variant="text" tone="paper">
						{isZh ? "了解递归森林" : "Explore Recursive Forest"}
					</MarketingLink>
				</div>
			</MarketingSection>
			...
			>
				...
				<div className="mt-10 lg:ml-[33.333%]">
					<MarketingLink href={isZh ? "/zh/geo" : "/geo"} variant="text" tone="paper">
						{isZh ? "了解我们的 GEO 方法" : "See our approach to GEO"}
					</MarketingLink>
				</div>
			</MarketingSection>
			...
			<MarketingSection tone="ink" innerClassName="py-24 sm:py-32 lg:py-40">
				<div className="grid gap-12 lg:grid-cols-12 lg:items-end">
					<div className="lg:col-span-8">
						...
						<h2 className="marketing-display mt-7 max-w-[14ch] text-[clamp(2.8rem,6vw,6.4rem)] leading-[0.94] font-medium tracking-[-0.05em] text-balance">
							{content.diagnostic.title}
						</h2>
						<p className="mt-8 max-w-[39rem] text-base leading-7 text-[var(--yonaris-paper)]/66 sm:text-lg">
							{content.diagnostic.body}
						</p>
					</div>
					<div className="lg:col-span-3 lg:col-start-10">
						<MarketingLink href={diagnosticPath} className="w-full sm:w-auto lg:w-full">
							{content.cta.primary}
						</MarketingLink>
					</div>
				</div>
