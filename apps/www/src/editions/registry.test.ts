...
import { GLOBAL_ENGLISH_SECTION_IDS, getEdition, findPublishedEditionPage } from "./registry";

const expectedPaths = ["/", "/product", "/approach", "/research", "/geo", "/company", "/diagnostic", "/privacy"];

describe("global English edition registry", () => {
	it("owns the complete English R0 route set as one edition", () => {
	...
	it("freezes each page-specific section spine", () => {
		...
		expect(GLOBAL_ENGLISH_SECTION_IDS.product).toEqual([
			"scope-rings-hero", "evidence-workbench", "responsibility-lanes", "scope-matrix", "request-close",
		]);
		expect(GLOBAL_ENGLISH_SECTION_IDS.approach).toEqual([
			"premise-hero", "four-step-path", "step-artifacts", "repeat-observation-boundary", "request-close",
		]);
		expect(GLOBAL_ENGLISH_SECTION_IDS.research).toEqual([
			"ledger-hero", "metric-anatomy", "cohort-comparison", "answer-annotation", "limits-and-request-close",
		]);
