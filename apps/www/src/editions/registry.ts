...
const englishPaths = ["/", "/product", "/approach", "/research", "/geo", "/company", "/diagnostic", "/privacy"] as const;
const englishPages = englishKeys.map((key, index): EditionPage => ({
	ref: `global-en:${key}`,
	editionId: "global-en",
	locale: "en",
...
const zhPages = zhKeys.map((key): EditionPage => ({
...
const editions: Record<SiteEdition, EditionDefinition> = {
	"global-en": {
		id: "global-en", home: "global-en:home", pages: englishPages,
		...
		localeFallbackHome: "global-en:home", analyticsPolicy: "disabled", diagnosticPolicy: "disabled",
	},
	"zh-cn-legacy": {
		id: "zh-cn-legacy", home: "zh-cn-legacy:home", pages: zhPages, primaryNavigation: [],
		footerNavigation: zhPages.map((page) => page.ref), localeFallbackHome: "zh-cn-legacy:home",
		analyticsPolicy: "disabled", diagnosticPolicy: "legacy-v1",
	},
};

export function getEdition(id: SiteEdition): EditionDefinition { return editions[id]; }
