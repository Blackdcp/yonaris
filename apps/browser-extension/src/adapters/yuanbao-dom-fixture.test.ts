...
describe("Yuanbao semantic DOM contract", () => {
	test("matches the current official composer, send control, and icon-only new-dialogue action", () => {
		const { document } = parseHTML(`<!doctype html><html><body>
			...
			<div class="agent-chat__bubble agent-chat__bubble--ai"><div class="agent-chat__speech-text"><div class="hyc-content-md hyc-content-md-done">Current answer</div></div><div>复制</div></div>
		</body></html>`);
