...
describe("Kimi browser-extension adapter", () => {
	test("declares the registered Kimi surface and adapter version", () => {
		expect(kimiSelectorContract).toMatchObject({
			version: "kimi-web-20260822-localpc-v11",
	...
	test("collects a structured answer, direct citation, and bounded screenshot region", async () => {
		...
		await expect(adapter.collectCurrentAnswer()).resolves.toMatchObject({
			...
			adapterVersion: "kimi-web-20260822-localpc-v11",
		});
	});

	test("observes an answer-scoped Kimi web-search tool without inventing query terms", async () => {
		const { document } = parseHTML(`<!doctype html><html><body>
			...
			<div class="segment-content-box" id="accepted-answer">
				<p>Current answer</p>
				<div class="toolcall-container toolcall-web_search" hidden>Provider search metadata</div>
		...
		await expect(
			...
			diagnostics: {
				extractorVersion: "kimi-search-evidence-20260822-v1",
				evidenceSource: "dom",
				searchBlockCount: 1,
