...
export const createBrandWithOrgFn = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		if (deployment.mode === "local" && !(await checkAnyOrgWriteAccess(session.user.id))) {
			throw new Error("Forbidden: This account cannot create brands");
		}
