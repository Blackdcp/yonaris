	...
	it("binds evidence upload to the confirmed runner session and approved adapter", async () => {
		...
		await expect(
			authorizeRunnerEvidenceUpload(
				"task-1",
				"stepfun",
				{ runnerSessionId: "session-current", adapterVersion: "doubao-web-20260821-localpc-v11" },
				principal,
				dependencies,
			),
		).resolves.toBe(task);
		await expect(
			authorizeRunnerEvidenceUpload(
				"task-1",
				"stepfun",
				{ runnerSessionId: "session-stale", adapterVersion: "doubao-web-20260821-localpc-v11" },
				principal,
				dependencies,
			),
	...
	it("accepts completion from the production-approved Doubao v8 adapter", async () => {
		await expect(
			authorizeExtensionTaskOperation({
				surfaceTargetKey: "doubao.consumer_web",
				readySurfaces: ["doubao.consumer_web"],
				operation: { kind: "complete", adapterVersion: "doubao-web-20260821-localpc-v11" },
			}),
		).resolves.toMatchObject({ surfaceTargetKey: "doubao.consumer_web" });
	});
