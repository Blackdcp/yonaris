...
const DEFAULT_APP_ICON = "/icons/yonaris-icon.svg";
...
function printBanner(): void {
	// Yonaris navy #0A1A2A → RGB(10, 26, 42)
	const navy = "\x1b[38;2;10;26;42m";
	const reset = "\x1b[0m";
	console.log(`${navy}${YONARIS_ASCII}${reset}`);
}
...
async function main() {
	...
	program
		.command("init")
		.description("set up a local Yonaris instance")
	...
	program
		.command("compose")
		.description("run Docker Compose commands using your Yonaris config")
	...
	program
		.command("upgrade")
		.description("upgrade your Yonaris deployment to this CLI's version")
...
async function runInit(options: InitOptions, version: string): Promise<void> {
	printBanner();
	p.intro(pc.bold("Setting up Yonaris"));
	...
	if (await fileExists(existingEnvPath)) {
		...
		if (!isElmoEnv) {
			p.log.warn(`A .env file already exists in ${configDir} and was not created by this CLI.`);
			...
			p.log.warn(`An existing Yonaris-compatible config was found at ${configDir}.`);
			const overwrite = await p.confirm({
	...
	// Yonaris does not have a first-party telemetry destination yet. Keep the
	// entire generated deployment opted out so no compatible upstream service
	// receives installation or usage events by default.
	...
	// This remains a no-op unless a future Yonaris-owned key and HTTPS endpoint
	// are explicitly configured and telemetry is enabled.
...
const BRIGHTDATA_URL = "https://brightdata.com/";
const OLOSTEP_URL = "https://olostep.com/";
const OXYLABS_URL = "https://oxylabs.io/";
...
async function configureProvidersInteractive(env: EnvMap): Promise<"recommended" | "custom"> {
	p.note(
		[
			"Yonaris needs two kinds of providers:",
			"",
			...
			"",
			"Pricing assumes Yonaris's default cadence (5 runs/day × 2 surfaces).",
		].join("\n"),
		"AI providers",
	);
...
async function collectScraperKey(scraper: "brightdata" | "olostep" | "oxylabs", env: EnvMap): Promise<void> {
	if (scraper === "brightdata") {
		p.log.info(`Sign up: ${link(pc.cyan(BRIGHTDATA_URL), BRIGHTDATA_URL)}`);
		...
		p.log.info(`Sign up: ${link(pc.cyan(OXYLABS_URL), OXYLABS_URL)}`);
		...
		p.log.info(`Sign up: ${link(pc.cyan(OLOSTEP_URL), OLOSTEP_URL)}`);
		const key = await p.password({
...
async function collectBrightData(env: EnvMap, targets: string[]): Promise<void> {
	...
	p.log.info(`Sign up and generate an API token: ${link(pc.cyan(BRIGHTDATA_URL), BRIGHTDATA_URL)}`);
	const key = await p.password({
		message: "BrightData API token",
		validate: (v) => (!v ? "Required" : undefined),
	});
...
async function collectOlostep(env: EnvMap, targets: string[]): Promise<void> {
	...
	p.log.info(`Grab an API key: ${link(pc.cyan(OLOSTEP_URL), OLOSTEP_URL)}`);
	const key = await p.password({
		message: "Olostep API key",
		validate: (v) => (!v ? "Required" : undefined),
	});
...
async function collectOxylabs(env: EnvMap, targets: string[]): Promise<void> {
	...
	p.log.info(`Sign up and create Web Scraper API credentials: ${link(pc.cyan(OXYLABS_URL), OXYLABS_URL)}`);
	const username = await p.text({
		message: "Oxylabs username",
		validate: (v) => (!v ? "Required" : undefined),
	});
...
async function finalizeScrapeTargets(
	...
	if (!deduped) {
		p.log.warn("No SCRAPE_TARGETS configured. Yonaris will not run scheduled checks until you set them.");
		p.log.info(`Upstream reference: ${link(pc.cyan(PROVIDERS_DOC_URL), PROVIDERS_DOC_URL)}`);
...
async function runUpgrade(options: UpgradeOptions, cliVersion: string): Promise<void> {
	printBanner();
	p.intro(pc.bold("Upgrading Yonaris"));
