...
Exception handling is exact, not open-ended. The existing read-only e2e project retains fourteen assertions from superseded contracts: eight in `content-negotiation.spec.ts` (seven require English `Language:` inside Chinese Markdown and one requires the former Human-canonical label); one English interaction assertion requires the rejected Home `Category / Purpose / Scope` ReadingLens instead of the binding fixed Human/Agent claim reader; one Chinese system assertion requires a two-row desktop grid instead of the binding six-position orbit while its mobile 44px/overflow/interaction checks remain valid; one reduced-motion assertion locates the explicitly retired `.site-06-hero__media img` instead of the approved CinematicField image; and three governed-404 assertions require the explicitly rejected AI-template `↗` suffix instead of the current accessible arrow-free Home link (two exact-link checks and one focus check). The localized app tests and the marketing matrix must pass and explicitly cover the replacement behavior. These exact fourteen stale assertions must not be satisfied by reintroducing English leakage, rejected visual structures, or rejected arrow glyphs. Any other e2e failure is a release blocker. Task 5 does not modify `e2e/**` because that would trigger the out-of-scope portal/LAS deployment.
