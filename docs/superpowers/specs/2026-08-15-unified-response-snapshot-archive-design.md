...
这项能力不改变 Yonaris 的 Visibility、Share of Voice、品牌提及、引用或覆盖率公式，也不把技术失败记成品牌未提及。
...
登录、验证码、页面漂移或不确定提交等技术失败继续进入现有人工处理流程，不生成 `prompt_run` 或回答快照。有效回答即使没有品牌提及，也正常生成快照并以 `brandMentioned=false` 进入 Yonaris 原有分母。
...
- 指标回归：有无快照、快照 pending/failed/expired 均不改变同一批 prompt runs 的 Yonaris M/S。
