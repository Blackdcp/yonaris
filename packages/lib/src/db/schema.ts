...
export const browserRunnerDevices = pgTable(
	...
	(table) => ({
		...
		validSurfaceCount: check(
			...
			sql`cardinality(${table.supportedSurfaces}) BETWEEN 1 AND 7`,
		),
		validSurfaces: check(
			"browser_runner_devices_valid_surfaces",
			sql`${table.supportedSurfaces} <@ ARRAY['doubao.consumer_web', 'deepseek.consumer_web', 'qwen.consumer_web', 'kimi.consumer_web', 'wenxin.consumer_web', 'yuanbao.consumer_web', 'zhipu.consumer_web']::text[]`,
		),
	}),
).enableRLS();
