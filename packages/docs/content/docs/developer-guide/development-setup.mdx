...
description: Set up a local development environment for contributing to Yonaris.
---
...
| `ELMO_ENCRYPTION_KEY` | Key for provider credentials stored in the database — `openssl rand -base64 32` |
...
| `DEEPSEEK_API_KEY` | For DeepSeek direct-API tracking |
...
Yonaris uses Ladle for component development in isolation:
