...
DeepSeek may issue several searches during one online request even when Elmo sends a low search-use hint. Treat the recorded fan-out count as the observed behavior, and monitor DeepSeek token and search spend accordingly.
