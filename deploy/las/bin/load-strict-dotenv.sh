...
dotenv_key_is_allowed() {
	case "$1" in
		...
		BROWSER_RUNNER_BOOTSTRAP_EXPIRES_AT | BROWSER_RUNNER_ID | \
		BROWSER_RUNNER_LOCALE | BROWSER_RUNNER_MARKET | BROWSER_RUNNER_TIMEZONE | \
		...
		IMAGE_REGISTRY | IMAGE_TAG | JINA_API_KEY | MISTRAL_API_KEY | OLOSTEP_API_KEY | \
